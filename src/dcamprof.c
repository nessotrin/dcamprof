         ...
         const char report_dir[])
{
    ...
    if (de > 0.1 && skip_observer_mapping) {
        elog(
            ...
            "  the profile. The profile may therefore not work in some ICC products,\n"
            "  and may not produce the expected colors.\n", de);
    }
    ...
    if (!skip_observer_mapping || de <= 0.1) {
        ...
        fm = m3x3_mul_m3x3(bradford_map_white(prof_d50, ref_d50), forward_matrix);
    }
...
#pragma omp parallel for
        ...
        if (has_suffix(argv[ai+1], ".dcp") || has_suffix(argv[ai+1], ".DCP")) {
            ...
            struct profio_icc *icc = make_icc(prof, prof->camera_name, skip_lut_in_report ? ICC_PROFILE_TYPE_MATRIX : ICC_PROFILE_TYPE_LABLUT, false, 33, NULL, 0, false, tc, tc_len, GC_NONE, TC_NEUTRAL, false, NULL, NULL);
            profio_icc_write(icc, argv[ai+1], true);
            profio_icc_delete(icc);
        } else {
            if (tc != NULL) {
        ...
        struct profio_icc *icc = make_icc(prof, camera_name, icc_type, apply_wb, clut_side, trc, trc_count, skip_observer_mapping, tc, tc_len, gc_type, tc_type, skip_tc_apply, ntro_conf, report_dir);
        free(tc);
        print_transfer_function(report_dir, trc, trc_count);
        dcam_profile_delete(prof);
        if (copyright != NULL) {
